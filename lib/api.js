...
const urlMapper = {
    ...
    original: (url) => 'http://knu.ua' + url,
    univ: (id) => `${urlMapper.root}/univs/${id}`,
    faculty: (id) => `${urlMapper.root}/faculties/${id}`,
    specialization: (id) => `${urlMapper.root}/specializations/${id}`,
    applicant: (id) => `${urlMapper.root}/applicants/${id}`,
};
...
const mapModel = {
    univ: function (object) {
        return {
            name: object.name,
            oUrl: urlMapper.original(object.url),
 ...
 * @param {String} query
 * @param {int} [limit=5]
 * @returns {Q.Promise.<{status, data: {univs, faculties, specs, applicants}}>}
 */
    ...
    return Q
        ...
        .then(async function (spec_id) {
            const [spec, applications] = await Q.all([
                ...
                Application.find({spec_id: spec_id}).sort({pos: 1})
            ]);
